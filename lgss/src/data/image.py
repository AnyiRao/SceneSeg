...
import sys
sys.path.append(".")
from utilis import strcal,read_txt_list,read_pkl,read_json
    ...
    def _get_single_item(self, ID):
        ...
        if 'image' in self.mode:
            for ind in self.shot_boundary_range:
                ...
                path = osp.join(self.data_root, 'keyf_240p/{}'.format(imdbid), name)
...
def data_pre(cfg):
    ...
    jobs = [Process(target=data_pre_one, \
...
def main():
    ...
    for batch_idx, (data_place,data_cast,data_act,data_aud,target) in enumerate(dataloader):
        # data_place = data_place.cuda()if 'image' in cfg.dataset.mode else []
